...
void idPlayer::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir,
	...
	/*
		Ed Conroy's Code
		------------------------------------------------------------------------
		Creates armor that allows the player to be invulnerable to all damage 
		for the duration of the armor
		------------------------------------------------------------------------
	*/
	if(inventory.armor <= 0)
	{
		inventory.armor = 0;
		inventory.armor_type = 0;
	}
	// give feedback on the player view and audibly when armor is helping
	else
	{
		if(inventory.armor_type == 1)
		{
			inventory.armor -= 3;
			return;
		}
